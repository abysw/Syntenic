...
#	$count2{$gene2{$l[1]}}+=1;
	...
	say "$gene1{$l[0]}\t$gene2{$l[1]}" if $gene2{$l[1]} eq "scaffold153846_cov67";
...
#		$chan2{$scaf{$_}}=$chan2{$scaf{$_}};
	}else{
		$chan2{$scaf{$_}}="$sps2$j";
		$j++;
	}
	#say $j;
	say OUT "$_\t$chan1{$_}\t$scaf{$_}\t$chan2{$scaf{$_}}\t$len{$_}";
#	`rm $sp1.bed.n1` if -f "$sp1.bed.n1";
#	`rm $sp2.bed.n2` if -f "$sp2.bed.n2";
	`grep $_ $bed1 >> $sp1.bed.n1 `;
	`grep $scaf{$_} $bed2 >> $sp2.bed.n2 `;
}
